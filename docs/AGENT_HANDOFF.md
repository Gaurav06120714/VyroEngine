...
cmake --build build
ctest --test-dir build              # 72 test suites, all green
```
Apps in `build/bin/`:
...
- **v6.0.0** — production-grade era (released): GPU instancing, depth-texture shadows, co-op gameplay, level obstacles, profiling (phases v5.1.0-v5.5.0).
  ...
  - **v5.4.0** ✅ V6.4 Level & Obstacle Pipeline (`ai::avoid_obstacles`; pillars)
  - **v5.5.0** ✅ V6.5 Profiling & Optimization (`FrameStats`; on-screen frame graph)
  - **v6.0.0** ✅ umbrella release (version bump, `cpack` tarballs, GitHub release)

> Tag scheme note: each version's phase tags continue the patch series one major
> behind (V4=`v3.x`, V5=`v4.x`, V6=`v5.x`); the umbrella release matches the major
> (`v4.0.0`, `v5.0.0`, `v6.0.0`). V7 phases will be `v6.1.0`…`v6.5.0`, release `v7.0.0`.

---

## 4. WHAT TO DO NEXT  (V4/V5/V6 done; V7 next — write docs/ROADMAP_V7.md)
...
| ✅ V10 Variety & Polish | done — V10.1 hazards, V10.2 boss phases, V10.3 weapon unlocks, V10.4 timed power-ups, V10.5 title menu (see `docs/ROADMAP_V10.md`); released as **v10.0.0**. | v9.1.0–v10.0.0 |
| **▶ V11 (TBD)** | **SCOPE NEXT** — V10 (variety & polish) shipped. Candidate V11 themes: a second biome/level + level select, a SceneSerializer-driven editor UI, controller support, online leaderboards, or a Vulkan feature-parity pass. Write `docs/ROADMAP_V11.md` before starting. | v10.1.0+ |
...
- 72 test suites, all green in Release.
- Latest tag: **v10.0.0** (V10 umbrella release; V10 complete).
- **Next action: scope V11** — write docs/ROADMAP_V11.md first.

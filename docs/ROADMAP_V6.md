...
| Phase | Goal | Tag |
|-------|------|-----|
| V6.1 — GPU Hardware Instancing | Add per-instance vertex attributes + an instanced draw path to the RHI/OpenGL backend (`glDrawElementsInstanced`, mat4 instance attribute at locations 4–7, divisor 1). Draw the whole horde with one instanced call from a per-instance transform buffer — no CPU re-batch each frame. Replaces the V5.5 CPU batch. | v5.1.0 |
